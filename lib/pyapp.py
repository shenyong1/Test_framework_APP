...
class Pyapp(Pyse):
    ...
    def taps(self,positions,duration=2):
        self.d.tap(positions,duration)
    ...
    def wait_and_save_exception(self, css, name):
        try:
            ...
            from conf.path import APPPICTURE_PATH
            import threading
            self.get_windows_img(APPPICTURE_PATH.format(threading.current_thread().getName()) + name + '.jpg')
            return False
